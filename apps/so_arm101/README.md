...
This application lives at `apps/so_arm101/`; see the [Applications catalog](../../README.md#applications) for how it fits alongside other applications in this repository.
...
These packages live in this directory and are built together with the shared sources in [`common/`](../../common/).

### External (imported via [`so_arm101.repos`](so_arm101.repos) and the shared [`common/common.repos`](../../common/common.repos), via `pixi run install-source-deps`)
...
Documentation specific to this application lives in [`docs/`](docs/).

| Guide                                                    | What it covers                                                         |
| -------------------------------------------------------- | ---------------------------------------------------------------------- |
| [Installation](docs/installation.md)                     | Requirements, Pixi install, manual install, `rmw_zenoh`                |
| [Development Guide](docs/development.md)                 | Pixi workflow, `pixi shell`, scene config, Gazebo troubleshooting      |
| [Running the Robot](docs/running-the-robot.md)           | Launching in Gazebo, MuJoCo, or on real hardware                       |
| [Teleoperation](docs/teleoperation.md)                   | Leader-arm, RViz interactive-marker IK, and phone (WebXR) teleop       |
| [Calibration](docs/hardware/calibration.md)              | Servo calibration via LeRobot, `joint_config_file`, known limitations  |
| [Udev Rules](docs/hardware/udev-rules.md)                | Stable device symlinks for arms and cameras                            |
| [Cameras](docs/hardware/cameras.md)                      | Wrist/static camera configuration, calibration files, and TF frames    |
| [End-to-End Pipeline](docs/demos/end-to-end-pipeline.md) | Record → Train → Deploy with Rosetta and LeRobot                       |
| [Pre-trained Policy](docs/demos/pretrained-demo.md)      | Skip the slow loop — run a pre-trained ACT policy in Gazebo in minutes |
| [MCP Interface](docs/mcp.md)                             | Control, introspect, or debug the robot from an AI agent via ROS-MCP   |

Repository-wide documentation (contributing, lint) is in
[`docs/`](../../docs/README.md) at the repository root.
